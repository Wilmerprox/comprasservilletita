...
// Using jsPDF library to generate PDF instead of CSV
function downloadPDF() {
    ...
    rows.forEach((row, index) => {
        ...
        if (category && product) {
            rowData.push([category, product, quantity, unit, price, total]);
        }
    });
    
    // Adding table data to PDF
    doc.autoTable({
        head: [['Categoría', 'Producto', 'Cantidad', 'Unidad de medida', 'Precio unitario', 'Precio total']],
        body: rowData
    });
